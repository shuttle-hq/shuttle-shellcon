...
pub async fn get_species(
    ...
    if let Some(name) = &params.name {
        if name.len() < 2 {
            return Err(ApiError::InvalidQuery(
                "Name search term must be at least 2 characters".to_string(),
            ));
        }
    }
    ...
    if let Some(scientific_name) = &params.scientific_name {
        if scientific_name.len() < 2 {
            return Err(ApiError::InvalidQuery(
                "Scientific name search term must be at least 2 characters".to_string(),
            ));
        }
    }
    ...
    tracing::info!(
        ...
        search_term = params
            .name
            .as_deref()
            .unwrap_or_else(|| params.scientific_name.as_deref().unwrap_or("")),
...
pub fn calculate_feeding_schedule(
    species: &Species,
    params: &FeedingScheduleParams,
